...
- **Pendientes cerrados**: (a) reset-safety — `columnSyncService` asegura `org_id` en defs scoped al materializar; (b) request_review cross-org — 403 si el caso es de otra org, 400 si el revisor no pertenece a la org.
- **Verificación adversarial (workflows, ultracode)** — sondas en paralelo intentando romper el aislamiento. Encontró 2 fugas reales (no detectadas por el E2E manual): (1) `GET /api/entities/:id` (getOne) no filtraba por org; (2) endpoints hermanos por `entity_id` (chatter, attachments) tampoco. **Corregido** con un helper central `services/orgScope.ts` (`callerCanAccessEntity`) aplicado a getOne, getList, chatter (GET/POST/reply), attachments (lista/file/POST) y entities PUT/DELETE. **Barrido final LIMPIO: 0 fugas** (getOne 404, chatter 403, attachments []/404, thread/queue/assignments/reminders vacíos cross-org; el paciente global sí cross-org, por diseño).
- Tests pasados: aislamiento chat/ficha por TODOS los endpoints, gating superadmin/admin-de-org, cross-org en derivación, anti-spoof de `org_id`, reset-safety, y demo visual en las 6 ventanas (switch de org en el dropdown).
